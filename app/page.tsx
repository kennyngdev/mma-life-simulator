...
import { advance, choiceRewardFor, difficulties, endingFor, eventFor, identityDetail, isComplete, newLife, performMove, phases, resolveBattle, sectFor, sects, selectTarget, startBattle, statNames, type DifficultyId, type LifeRun, type LifeScreen, type SectId } from './life-engine';
...
export default function DaxiaPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try { const parsed = JSON.parse(saved) as { screen: LifeScreen; run: LifeRun }; if (parsed.run?.version === 4 && parsed.screen !== 'start') { setRun(parsed.run); setScreen(parsed.screen); } else { window.localStorage.removeItem(SAVE_KEY); } } catch { window.localStorage.removeItem(SAVE_KEY); }
    }, 0);
