...
import { advance, choiceChanceDetailFor, choiceChanceFor, choiceCommitmentFor, choiceFailureFor, choiceRewardFor, chooseBattleUpgrade, chooseInsight, difficulties, endingFor, eventFor, hasTalent, identityDetail, identityRarity, insightChoicesFor, insightDefinitions, insightThresholds, isComplete, newLife, nextInsightTier, performMove, phases, resolveBattle, resolvedSectFor, rulesFor, sectFor, sects, selectTarget, startBattle, statNames, traits, upgradeChoicesFor, type DifficultyId, type InsightId, type LifeRun, type LifeScreen, type RarityId, type SectId, type UpgradeId } from './life-engine';
...
export default function DaxiaPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try { const parsed = JSON.parse(saved) as { screen: LifeScreen; run: LifeRun }; if (parsed.run?.version === 13 && parsed.screen !== 'start') { setRun(parsed.run); setScreen(parsed.screen); } else { window.localStorage.removeItem(SAVE_KEY); } } catch { window.localStorage.removeItem(SAVE_KEY); }
    }, 0);
    return () => window.clearTimeout(timer);
  }, []);
  ...
  const continueAfterResult = () => { if (!run) return; if (run.pendingUpgrade) { setRun({ ...run, result: null }); setScreen('upgrade'); return; } if (nextInsightTier(run)) { setScreen('insight'); return; } if (isComplete(run)) { setRun((previous) => previous ? { ...previous, result: null } : previous); setScreen('ending'); return; } setRun((previous) => previous ? { ...previous, result: null } : previous); setScreen('life'); };
  const selectUpgrade = (id: UpgradeId) => { if (!run) return; const next = chooseBattleUpgrade(run, id); if (next === run) return; setRun(next); if (nextInsightTier(next)) setScreen('insight'); else if (isComplete(next)) setScreen('ending'); else setScreen('life'); sound('win'); };
  ...
  return <div className={styles.shell}><Header screen={screen} run={run} onRestart={restart} onOpenCharacter={() => setCharacterOpen(true)} characterOpen={characterOpen} />
    ...
    {screen === 'upgrade' && run && <UpgradeView run={run} onChoose={selectUpgrade} />}
