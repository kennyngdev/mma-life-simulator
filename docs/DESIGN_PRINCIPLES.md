...
- Combat timeline progress is elapsed-time driven at a fixed 60 Hz simulation cadence. Existing speed values retain their former 450 ms calibration, but actors move in fractional real-time steps and resolve at the exact earliest threshold so display refresh rate cannot alter deterministic turn order. Backgrounded play does not fast-forward on return.
