...
- Every victory creates one mandatory post-battle choice from three distinct permanent combat upgrades. Upgrade identities and 60/30/10 rarity rolls are derived from the run seed and completed turn; an offer always contains at least two rarity tiers, cannot be rerolled, and is resolved before an insight, the next event, or the ending. Defeat remains terminal and grants no upgrade.
...
- Seeds support replay, comparison, sharing, and debugging. Reproduction includes the selected inherited trait as well as the game version, seed, identity, difficulty, event-specific choice ids, battle upgrades, and insights. Save compatibility is explicit through the run version; inherited talents begin at save version 9, the reviewed rarity tiers at version 10, committed battle intents at version 11, dynamic choices plus 門派造詣 at version 12, and permanent post-battle upgrades at version 13. Earlier in-progress runs restart while the separate 前世見聞 collection remains intact.
...
- Each battle draws every actor's starting timeline position independently from 8–70 using the saved seed and RNG index. Equal positions are legal; stable actor-array order resolves exact ties so replays remain deterministic.
